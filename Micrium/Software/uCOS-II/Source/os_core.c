        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioCur != OS_TASK_IDLE_PRIO) {
                    if (TaskParameter[OSPrioCur].TaskCurrentExecutionTime != 0) {
                        OSPrioHighRdy = OSPrioCur;
                        OSTCBHighRdy = OSTCBPrioTbl[OSPrioCur];
                    }
                    if (TaskParameter[OSPrioCur].TaskCurrentExecutionTime == 0 && OSPrioHighRdy != OSPrioCur) {    /*讓被preemp的task做完剩下的部分*/
                        OSPrioHighRdy = OSPrioCur;
                        OSTCBHighRdy = OSTCBPrioTbl[OSPrioCur];
                    }
                }
                    ...
                    if ((Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a")) == 0) {
                        if (OSPrioCur == OS_TASK_IDLE_PRIO) {
                            printf("%2d\tPreemption\ttask(%2d)\ttask(%2d)(%2d)\n",
                                OSTimeGet(), OSPrioCur, TaskParameter[OSPrioHighRdy].TaskID, TaskParameter[OSPrioHighRdy].TaskNumber);
                            fprintf(Output_fp, "%2d\tPreemption\ttask(%2d)\ttask(%2d)(%2d)\n", 
                                OSTimeGet(), OSPrioCur, TaskParameter[OSPrioHighRdy].TaskID, TaskParameter[OSPrioHighRdy].TaskNumber);
                        }
                        else {
                            printf("%2d\tPreemption\ttask(%2d)(%2d)\ttask(%2d)(%2d)\n", 
                                OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskNumber, TaskParameter[OSPrioHighRdy].TaskID, TaskParameter[OSPrioHighRdy].TaskNumber);
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        TaskParameter[OSPrioCur].TaskCurrentExecutionTime--;
        
        for (int i = 0; i < TASK_NUMBER; i++) {
            if ((OSTimeGet() + TaskParameter[i].TaskPeriodic - TaskParameter[i].TaskArriveTime) % TaskParameter[i].TaskPeriodic == 0 && 
                (OSTimeGet()-TaskParameter[i].TaskArriveTime)/TaskParameter[i].TaskPeriodic > TaskParameter[i].TaskNumber) {
                if (TaskParameter[i].TaskCurrentExecutionTime != 0) {
                    if (TaskParameter[OSPrioCur].TaskCurrentExecutionTime == 0) {
                        missdeadline = i;
                    }
                    else {
                        if ((Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a")) == 0) {
                            printf("%2d\tMissDeadline\ttask(%2d)(%2d)\t-------------------\n", OSTimeGet(), TaskParameter[i].TaskID, TaskParameter[i].TaskNumber);
        ...
        //printf("Time %d taskID %d current executiontime %d\tResponse Time %d\n", OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskCurrentExecutionTime, OSTimeGet() - (TaskParameter[OSPrioCur].TaskPeriodic * TaskParameter[OSPrioCur].TaskNumber + TaskParameter[OSPrioCur].TaskArriveTime));
    }
}
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            INT16U min_task_time = SYSTEM_END_TIME;
            INT16U min_task_ID = OS_TASK_IDLE_PRIO;
            
            while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {
                int next_arrive_time = TaskParameter[ptcb->OSTCBPrio].TaskPeriodic * TaskParameter[ptcb->OSTCBPrio].TaskNumber + TaskParameter[ptcb->OSTCBPrio].TaskArriveTime;
                if (ptcb->OSTCBDly == 0) {
                    if (next_arrive_time < min_task_time && TaskParameter[ptcb->OSTCBPrio].TaskID < min_task_ID) {
                        OSPrioHighRdy = ptcb->OSTCBPrio;
                        min_task_ID = TaskParameter[ptcb->OSTCBPrio].TaskID;
                        min_task_time = next_arrive_time;
                    }
                }
                ptcb = ptcb->OSTCBNext;
            }
            ...
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
            int response_time = OSTimeGet() - (TaskParameter[OSPrioCur].TaskPeriodic * TaskParameter[OSPrioCur].TaskNumber + TaskParameter[OSPrioCur].TaskArriveTime);
            int preemption_time = response_time - TaskParameter[OSPrioCur].TaskExecutionTime;
            int os_time_delay = TaskParameter[OSPrioCur].TaskPeriodic - response_time;
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if (TaskParameter[OSPrioCur].TaskCurrentExecutionTime == 0) {
                    if ((Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a")) == 0) {
                        if (OSPrioHighRdy == OS_TASK_IDLE_PRIO) {
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t", OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskNumber, OSPrioHighRdy);
                            fprintf(Output_fp, "%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)\t", OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskNumber, OSPrioHighRdy);
                        }
                        else {
                            printf("%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t", OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskNumber, TaskParameter[OSPrioHighRdy].TaskID, TaskParameter[OSPrioHighRdy].TaskNumber);
                            fprintf(Output_fp, "%2d\tCompletion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t", OSTimeGet(), TaskParameter[OSPrioCur].TaskID, TaskParameter[OSPrioCur].TaskNumber, TaskParameter[OSPrioHighRdy].TaskID, TaskParameter[OSPrioHighRdy].TaskNumber);
                        }
                        printf("       %d\t       %d\t      %d\n", response_time, preemption_time, os_time_delay);
                        fprintf(Output_fp, "       %d\t       %d\t      %d\n", response_time, preemption_time, os_time_delay);
                        TaskParameter[OSPrioCur].TaskNumber++;
                        TaskParameter[OSPrioCur].TaskCurrentExecutionTime = TaskParameter[OSPrioCur].TaskExecutionTime;
                        fclose(Output_fp);
                    }
                }
                
                if (missdeadline !=  TASK_NUMBER + 1) {
                    if ((Output_err = fopen_s(&Output_fp, OUTPUT_FILE_NAME, "a")) == 0) {
                        printf("%2d\tMissDeadline\ttask(%2d)(%2d)\t-------------------\n", OSTimeGet(), TaskParameter[missdeadline].TaskID, TaskParameter[missdeadline].TaskNumber);
                        fprintf(Output_fp, "%2d\tMissDeadline\ttask(%2d)(%2d)\t-------------------\n", OSTimeGet(), TaskParameter[missdeadline].TaskID, TaskParameter[missdeadline].TaskNumber);
                        fclose(Output_fp);
                    }
                    OSRunning = OS_FALSE;
