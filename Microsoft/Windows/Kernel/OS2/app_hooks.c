...
void InputFile() {
    ...
    */
    ...
    while (!feof(fp))
    {
        ...
        TaskParameter[j].TaskCurrentExecutionTime = TaskParameter[j].TaskExecutionTime;
        j++;
    }
    
    for (int i = 1; i < TASK_NUMBER; i++) {
        task_para_set TaskParameter_current = TaskParameter[i];
        int j = i-1;
        while (j >= 0 && TaskParameter[j].TaskArriveTime > TaskParameter_current.TaskArriveTime) {
            TaskParameter[j + 1] = TaskParameter[j];
            j--;
        }
        TaskParameter[j + 1] = TaskParameter_current;
    }
    ...
    /*
    for (int i = 0; i < TASK_NUMBER; i++) {
        printf("TaskID:%d\tTask Arrive Time:%d\tTask Periodic:%d\tTask Execution Time:%d\tTask Priority:%d\n",TaskParameter[i].TaskID, TaskParameter[i].TaskArriveTime, TaskParameter[i].TaskPeriodic, TaskParameter[i].TaskExecutionTime, TaskParameter[i].TaskPriority);
    }*/
    fclose(fp);
    /*read file*/
}
